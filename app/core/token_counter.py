...
def count_tokens(text: str) -> int:
    ...
    if encoding is not None:
        ...
        # encode()는 텍스트를 토큰 ID 리스트로 변환
        # 리스트의 길이 = 토큰 수
        return len(encoding.encode(text))  # TODO: 이 줄을 삭제하고 구현하세요
    else:
        # tiktoken 없으면 근사치 계산
        # 영어: 약 4글자당 1토큰
...
def count_messages_tokens(messages: list[BaseMessage]) -> int:
    ...
    for message in messages:
        ...
        # [지식] chatml 형식과 메시지 오버헤드
        # llm api 는 메시지를 chatml 형식으로 변환하여 전송함
        ...
        # 이 형식 때문에 메시지당 약 4토큰의 오베헤드가 발생:
        # <|im_start|> : 1토큰 (특수 토큰)
        # role : 1토큰 (user, assistant , system)
        # 줄바꿈(\n) : 1토큰
        # <|im_end|> : 1토큰 (특수 토큰)
        # 합계 4토큰
        # 이 값은 openai upstage api의 실제 측정값 기반임
