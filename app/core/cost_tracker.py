...
class CostTracker:
    ...
    async def _send_discord_alert(
        ...
        Args:
            ...
            color: 임베드 색상 (16진수)
        """
        # [지식]Discord Webhook URL 설정
        # 디스코드 채널에 메시지를 보내는 간단한 방법
        # 별도 봇 설정 불필요(webhook url만 있으면 됨)
        # slack, teams 등도 유사한 webhook 기능 제공
        # 운영 알림, 에러 알림 등에 활용
        #
        # 설정 방법 : 디스코드 채널 설정 -> Integrations -> Webhooks -> New Webhook
        # 생성된 webhook url을 .env 파일에 추가
...
# 싱글톤 패턴을 사용하여 앱 전체에서 동일한 인스턴스를 공유
# 일일 알림 생태(_already_alerted_today) 공유
# supabase 클라이언트 재사용
#
