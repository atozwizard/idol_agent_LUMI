...
테이블 구조 (conversations):
    - id : UUID (PK)
    - session_id : TEXT (세션식별자)
    - user_id : TEXT (사용자 식별자, nullable
    - role : TEXT (user, assistant, system)
    - content : TEXT (메시지 내용)
    - intent : TEXT (chat, rag, tool)
    - tool_name: TEXT (사용된 tool)
...
# [지식] 대화저장 VS 체크포인터
# 1. 체크포인터 (langgraph 내장)
#   - 목적 : 그래프 상태 복원 (대화 이어기기)
#   - 저장 : 전체 state 객체 (직렬화된 상태)
#   - 용도 : 런타임에 이전 상태 로드
#
# 2. 대화 저장 (이 모듈)
#     - 목적 : 대화 분석, 디버깅, 감사로그
#     - 저장 : 개별 메시지 (사람이 읽기 쉬운 형태)
#     - 용도 : 대시보드, 분석, 문제해결, GDPR 대응
#
# 둘 다 필요한 이유:
#     - 체크포인터만으로는 대화 내용 검색/분석 어려움
#     - 대화 저장만으로는 그래프 상태 복원 불가
